...
class Vgg16_HED():
    ...
    def __init__(self, cfgs, run='training'):
        ...
        self.inputs = tf.placeholder(tf.float32, [None, 1, self.cfgs[run]['data_width'], self.cfgs[run]['n_channels']])
        self.groundtruths = tf.placeholder(tf.float32, [None , 1, self.cfgs[run]['data_width'], 1])
    ...
    def side_layer(self, inputs, name, upscale):
        """
        	https://github.com/s9xie/hed/blob/9e74dd710773d8d8a469ad905c76f4a7fa08f945/examples/hed/train_val.prototxt#L122
             conv followed with Deconvoltion layer to upscale the size of input image sans color
        """
        with tf.variable_scope(name):
