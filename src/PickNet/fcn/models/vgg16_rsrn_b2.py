...
class Vgg16_RSRN():
    ...
    def define_model(self):
        ...
        """
        HED model based on VGG16 model without FC layers
        ...
        self.io.print_info('Main Structure OK')
        #RSRN in a deep to shallow way
        #block 5
        ...
        self.fuse = self.conv_layer(tf.concat(self.side_outputs, axis=3),
                                    ...
                                    w_init=tf.constant_initializer(1/16))    
    ...
    def RU_layer(self, side_inputs, res_inputs, name, upscale_out, upscale_res):
        self.io.print_info(side_inputs.shape.as_list())
        self.io.print_info(res_inputs.shape.as_list())
        ...
        temp_outputs = self.conv_layer(x = classifier, W_shape = w_shape, b_shape=1,
                                       w_init=tf.constant_initializer(),
                                       b_init=tf.constant_initializer(),name=name + '_RU')
        
        res_outputs = temp_outputs[:,:,:,1:2]
        
        side_outputs = self.deconv_layer(x = temp_outputs[:,:,:,0:1],
                                         upscale=upscale_out,
