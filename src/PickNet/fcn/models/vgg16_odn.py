# -*- coding: utf-8 -*-
"""
Created on Sun Oct 14 19:09:31 2018
...
class Vgg16_ODN():
    ...
    def __init__(self, cfgs, run='training'):
        ...
        if run == 'training':
            self.batch_size = self.cfgs['batch_size_train']
            self.n_collects = self.cfgs[run]['n_collects']
        else:
            self.batch_size = self.cfgs['batch_size_test']
            self.n_collects = 1
            
        self.data_width =  self.cfgs[run]['data_width']
        
        self.io.print_info('Start Defining Model')
    ...
    def define_model(self):
        ...
        self.side_5, self.residual_5 = self.side_layer_bottom(self.conv5_3,'side_5',16,self.cfgs['b5_sout'])
        
        self.side_4, self.residual_4 = self.side_layer_srn(self.conv4_3, self.residual_5, 'side_4',8,self.cfgs['b4_sout'])
        
        self.side_3, self.residual_3 = self.side_layer_srn(self.conv3_3, self.residual_4, 'side_3',4,self.cfgs['b3_sout'])
        
        self.side_2, self.residual_2 = self.side_layer_srn(self.conv2_2, self.residual_3, 'side_2',2,self.cfgs['b2_sout'])
        
        self.side_1 = self.side_layer_top(self.conv1_2, self.residual_2, 'side_1',self.cfgs['b1_sout'])
               
        self.io.print_info('Start Defining ODN')
        
        self.odn_1 = self.odn_layer(self.side_1, self.cfgs['ODN_ksize'], 'odn_1', 1)
        self.odn_2 = self.odn_layer(self.side_2, self.cfgs['ODN_ksize'], 'odn_2', 1)
        self.odn_3 = self.odn_layer(self.side_3, self.cfgs['ODN_ksize'], 'odn_3', 2)
        self.odn_4 = self.odn_layer(self.side_4, self.cfgs['ODN_ksize'], 'odn_4', 4)
        self.odn_5 = self.odn_layer(self.side_5, self.cfgs['ODN_ksize'], 'odn_5', 8)
        
        self.side_outputs = [self.odn_1, self.odn_2, self.odn_3, self.odn_4, self.odn_5]
        
        
        self.io.print_info('Start Defining Side ODN')
        
        self.fuse = self.conv_layer(tf.concat(self.side_outputs, axis=3),
                                    [1, 1, len(self.side_outputs), self.cfgs['fuse_sout']], name='fuse', use_bias=False,
                                    w_init=tf.constant_initializer(0.2))
        
        self.io.print_info('Done Defining ODN')
        
        self.odn_fuse = self.odn_layer(self.fuse, self.cfgs['ODN_ksize'], 'odn_fuse', 16)
        
        self.io.print_info('Added FUSE layer')
        
        self.outputs = self.side_outputs + [self.odn_fuse]
        """
        self.side_outputs = [self.side_1, self.side_2, self.side_3, self.side_4, self.side_5]
        self.fuse = self.conv_layer(tf.concat(self.side_outputs, axis=3),
                            [1, 1, len(self.side_outputs), 1], name='fuse', use_bias=False,
                            w_init=tf.constant_initializer(0.2))
    ...
    def deconv_layer(self, x, upscale, name, padding='SAME', w_init=None):
        ...
        w_shape = [1, upscale * 2, in_shape[-1], in_shape[-1]]
    ...
    def side_layer_bottom(self, inputs, name, upscale, sout):
        """
        https://arxiv.org/pdf/1703.02243.pdf
        """
        with tf.variable_scope(name):
            in_shape = inputs.shape.as_list()
            w_shape = [1, 3, in_shape[-1], sout]
        ...
        residual = classifier[:,:,:,int(sout/2):]
        
        side_output = self.deconv_layer(classifier[:,:,:,0:int(sout/2)],
                                        upscale=upscale,
    ...
    def side_layer_top(self, inputs, inputs_r, name, sout):
        """
        https://arxiv.org/pdf/1703.02243.pdf
        """
        with tf.variable_scope(name):
        ...
        classifier = self.conv_layer(classifier, [1,3,sout*2,1], b_shape=1,
                     w_init=tf.truncated_normal_initializer(stddev=0.1),
                     b_init=tf.constant_initializer(),
                     name=name + '_reduction')
    ...
    def side_layer_srn(self, inputs, inputs_r, name, upscale, sout):
        """
        https://arxiv.org/pdf/1703.02243.pdf
        """
        with tf.variable_scope(name):
        ...
        classifier = self.conv_layer(classifier, [1,3,sout*2,sout], b_shape=1,
                                     w_init=tf.truncated_normal_initializer(stddev=0.1),
                                     b_init=tf.constant_initializer(),
                                     name=name + '_reduction')
    ...
    def odn_layer(self, inputs, kernal_size, name, channel_num):
        
        with tf.variable_scope(name):
            w_shape = [1,kernal_size*10,channel_num*kernal_size,1]
        
        decomp_mat = np.zeros([self.batch_size*self.n_collects,1,self.data_width,kernal_size],dtype='float32')
        
        for idx in range(kernal_size):
            for jdx in range(self.data_width):
                if jdx%kernal_size == 0:
                    decomp_mat[:,:,jdx+idx,idx] = 1
        
        decomposition_mat = tf.constant(decomp_mat,name=name+'decomp_mat')
        
        decomp_input = []
        for idx in range(channel_num):
            decomp_input.append(tf.multiply(inputs[:,:,:,idx:idx+1],decomposition_mat[:,:,:,:],name='name_decomp_mult'))
        """
        #decomposition
        decom_input = []
        
        for idx in range(kernal_size):
            decom_input.append(inputs[:,:,:,:]*decomposition_mat[:,:,:,idx:idx+1])
            tf.concat(decom_input, axis=3, name = name+'_concat')
        """
        #do 3*kernal_size filter
        odn_output = self.conv_layer(tf.concat(decomp_input, axis=3, name = name+'_concat'),
                                     w_shape, b_shape=1,
                                     w_init=tf.truncated_normal_initializer(stddev=1/kernal_size),
                                     b_init=tf.constant_initializer(),name=name+'_odnout') 
    ...
    def setup_training(self, session):
        ...
        fuse_output = tf.nn.sigmoid(self.odn_fuse, name='fuse')
        fuse_cost = sigmoid_cross_entropy_balanced(self.odn_fuse, self.groundtruths, name='cross_entropy_fuse')
