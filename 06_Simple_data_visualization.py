...
if __name__ == '__main__':
    ...
    os.chdir(task_dir)
    if os.path.exists('./VISRes'):
        pass
    else:
        os.makedirs('./VISRes')
    # visualize EqT results         
    vis_key = cfgs['TASKID']
    e_list = create_e_dict_from_sum_csv('{}/eqt_hypoInverse.sum'.format(cfgs['REAL']['eqt_catalog_dir']), cfgs)
    ...
    e_list = create_e_dict_from_sum_csv('{}/seqt_hypoInverse.sum'.format(cfgs['REAL']['seqt_catalog_dir']), cfgs)
    ...
    # visualize PickNet results         
    e_list = create_e_dict_from_sum_csv('{}/picknet_hypoInverse.sum'.format(cfgs['REAL']['picknet_catalog_dir']), cfgs)
    ...
    plt.title('PickNet {} earthquakes TASK {}'.format(len(e_lon), vis_key))
    plt.plot([PRO_SX,PRO_EX],[PRO_SY,PRO_EY],color='b', linewidth=1.5)
    plt.scatter(e_lon,e_lat,color='r',label='Detected Earthquakes')
    plt.xlim([MINLON,MAXLON])
    plt.ylim([MINLAT,MAXLAT])
    ...
    m.drawmeridians(np.arange(114,120,1.0),labels=[0,0,0,1],fontsize=14)
    plt.legend()
    plt.savefig('./VISRes/picknet_HypoInverse_Horizontal.png',dpi=300)
    plt.close()
    ...
    plt.ylabel('Depth (km)')
    plt.gca().set_aspect(ratio)
    plt.savefig('./VISRes/picknet_HypoInverse_W-E_Vertical.png',dpi=300)
    plt.close()

    plt.figure(figsize=(12,4))
    plt.scatter(e_lat,e_dep)
    plt.title('PickNet {} earthquakes TASK {}'.format(len(e_lat),vis_key))
    plt.xlim([MAXLAT,MINLAT])
    plt.ylim([MAXDEP,0.0])
    plt.xlabel('Latitude')
    plt.ylabel('Depth (km)')
    plt.gca().set_aspect(ratio)
    plt.savefig('./VISRes/picknet_HypoInverse_N-S_Vertical.png',dpi=300)
    plt.close()

    visualize_profile(cfgs, PRO_SY, PRO_SX, PRO_EY, PRO_EX, e_lat, e_lon, e_dep, name_flag = 'PickNet')
